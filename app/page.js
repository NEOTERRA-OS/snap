...
function Capture({ uid, onDone, onClose, inbound, onInboundHandled }) {
  ...
  // Automatische Formaterkennung: Bilder/PDF → OCR, CSV/XLSX → Import.
  ...
  function smartDrop(e) { e.preventDefault(); setDrag(false); routeFiles(Array.from(e.dataTransfer?.files || [])); }
  function onPickAny(e) { routeFiles(Array.from(e.target.files || [])); e.target.value = ""; }
...
function Receipts({ uid, onOpen, q = "", setQ = () => {}, allScope = false, who = "", onCapture = null, notiUnread = 0, onNotis = null }) {
  ...
  const doExport = () => {
    const head = ["Belegnummer", "Datum", "Händler", "Rechnungsnr", "Mitarbeiter", "Kategorie", "Status", "Betrag", "Währung"];
    const esc = (x) => `"${String(x ?? "").replace(/"/g, '""')}"`;
    const lines = sorted.map((r) => [r.receipt_no || "", r.doc_date || "", r.merchant || "", r.invoice_no || "", names[r.user_id] || "", catInfo(r.category).label, STATUS[r.status] || r.status, r.gross ?? "", r.currency || ""].map(esc).join(","));
    ...
    <>
      ...
      <div className="neos nstat rx-mobile">
        ...
        <div className="nstat-kpis">
          <div className="nstat-kpi"><span className="cap">{t("Volumen")}</span><div className="nstat-v">{mN(total)} <span className="nstat-u">{curUnit}</span></div>{dlt(volDelta)}</div>
          ...
          <div className="nstat-kpi"><span className="cap">{t("Ø Betrag")}</span><div className="nstat-v">{mN(avg)} <span className="nstat-u">{curUnit}</span></div>{dlt(avgDelta)}</div>
          <div className="nstat-kpi"><span className="cap">{t("Vorsteuer")}</span><div className="nstat-v">{mN(vat)} <span className="nstat-u">{curUnit}</span></div><span className="nstat-d pos">{t("abzugsfähig")}</span></div>
        </div>
        <div className="nstat-cur"><button type="button" className={cur === "EUR" ? "on" : ""} onClick={() => setCur("EUR")}>EUR</button><button type="button" className={cur === "RON" ? "on" : ""} onClick={() => setCur("RON")}>RON</button></div>
        <div className="nstat-card">
          <div className="nstat-card-h"><b>{t("Nach Kategorie")}</b><span>{t("Brutto-Volumen")}</span></div>
          {catArr.length > 0 && (
            <div className="nstat-story"><Icon name="trend" size={13} /> {t("Größter Posten")}: {catArr[0].label} · {mN(catArr[0].sum)} {curUnit} ({Math.round(catArr[0].sum / catTotal * 100)}%)</div>
          )}
          {catArr.slice(0, 6).map((c, i) => (
            <div className="nstat-bar" key={c.label}>
              <div className="nstat-bar-top"><span className="nstat-bar-l"><Icon name={c.icon} size={14} /> {t(c.label)}</span><span className="nstat-bar-v">{mN(c.sum)} <span className="nstat-u">{curUnit}</span></span></div>
      ...
      {/* Alles untereinander (keine ANSICHTEN-Rail) */}
      ...
      <div className="kpis kx">
        <div className="kpi">
          ...
          <div className="n mono posv"><span className="nv">{mN(total)}</span><span className="nu">{curUnit}</span></div>
          {volDelta == null
        ...
        <div className="kpi">
          ...
          <div className="n mono"><span className="nv">{mN(avg)}</span><span className="nu">{curUnit}</span></div>
      ...
      <div className="kpis kx" style={{ marginTop: 14 }}>
        <div className="kpi">
          <div className="kt"><Icon name="checkcheck" />{t("Gebucht")}</div>
          <div className="n mono posv"><span className="nv">{mN(sum(booked))}</span><span className="nu">{curUnit}</span></div>
          <div className="ksub neu">{bookedPct}% {t("des Volumens")}</div>
        </div>
        <div className="kpi">
          <div className="kt"><Icon name="wallet" />{t("Offene Erstattung")}</div>
          <div className={"n mono" + (openReimb.length ? " warnv" : "")}><span className="nv">{mN(sum(openReimb))}</span><span className="nu">{curUnit}</span></div>
          <div className={"ksub " + (openReimb.length ? "warn" : "neu")}>{openReimb.length} {t("privat · erstattungsfähig")}</div>
        </div>
        <div className="kpi">
          <div className="kt"><Icon name="layers" />{t("Vorsteuer")}</div>
          <div className="n mono"><span className="nv">{mN(vat)}</span><span className="nu">{curUnit}</span></div>
          <div className="ksub neu">{t("abziehbar")} ({curUnit})</div>
        </div>
      </div>
      <div className="fxnote"><Icon name="banknote" size={12} /> {cur === "RON" ? `${t("Beträge in RON")} · 1 EUR = ${fmtN(RON_RATE)} RON` : t("Beträge in EUR · EZB-Kurs zum Belegdatum")}{unconverted > 0 ? ` · ${unconverted} ${t("ohne Kurs")}` : ""}</div>

      <div className="agrid">
        <div className="panel">
          <div className="pw">{t("Ausgaben pro Monat")}<span className="pw-hint">Ø {mC(total / (months.length || 1))}</span></div>
          {months.length === 0
            ? <div className="empty"><Icon name="trend" size={26} /><p>{t("Keine Daten im Filter.")}</p></div>
            : <MonthlyChart months={months} data={byMonth} mult={cur === "RON" ? RON_RATE : 1} />}
        </div>
        <Bars map={byCat} label="Top-Kategorien" limit={8} keyOf={keyCat} />
      </div>
      <div className="agrid" style={{ marginTop: 14 }}>
      ...
      <div className="agrid" style={{ marginTop: 14 }}>
        <Bars map={byMerch} label="Top-Lieferanten" limit={8} keyOf={keyMerch} />
        <div className="panel">
          ...
          {curs.length === 0
            ...
            : curs.map(([c, v], i) => (
              <div className="rrow" key={c}>
                ...
                <div className="rrgt"><span className="rv">{mC(v.eur)}</span><span className="rpct">{money(v.orig, c)}</span></div>
              </div>))}
        </div>
      </div>
      <div className="panel" style={{ marginTop: 14 }}>
        ...
        {sorted(byPay).map(([k, v], i) => { const mx = total || 1; return (
          <div className="rrow rrow-clk" key={k} onClick={() => setDrill({ title: t(k), predicate: (r) => (r.payment_method === "private" ? "Privat verauslagt" : "Firmenkarte") === k })} title={t("Belege anzeigen & Zahlart ändern")}>
            ...
            <div className="rrgt"><span className="rv">{mC(v)}</span><span className="rpct">{Math.round(v / mx * 100)}%</span><Icon name="chevronleft" size={14} style={{ transform: "rotate(180deg)", color: "var(--muted2)" }} /></div>
          </div>); })}
      </div>
      </>)}
...
function Admin({ session, only }) {
  ...
  return (
    <>
      ...
      <div className="adm-sec">
        <div className="adm-sec-h">
          <span className="lbl">{t("Kostenstellen")}</span>
          <button className="adm-cta" onClick={() => setShowNewCc((v) => !v)}><Icon name={showNewCc ? "x" : "plus"} size={14} /> {t("Kostenstelle anlegen")}</button>
        </div>
        {showNewCc && (
          <div className="card">
            <form onSubmit={addCc}>
        ...
        {ccList === null ? <div className="center" style={{ minHeight: 60 }}><span className="spin" /></div>
          : ccList.length === 0 ? <p className="hint">{t("Noch keine Kostenstellen — lege oben die erste an.")}</p> : (
          <div className="adm-grid">
            {ccList.map((cc) => {
              const editing = ccEdit?.id === cc.id;
              return (
                <div className="adm-card adm-cc" key={cc.id} style={cc.active ? undefined : { opacity: 0.55 }}>
                  {editing ? (
                    <>
                      <input className="mono" value={ccEdit.code} onChange={(e) => setCcEdit({ ...ccEdit, code: e.target.value })} placeholder={t("Code")} style={{ width: 84, height: 32, padding: "0 8px" }} />
                      <input value={ccEdit.name} onChange={(e) => setCcEdit({ ...ccEdit, name: e.target.value })} style={{ flex: 1, height: 32, padding: "0 8px" }} />
                      <button type="button" className="icobtn" onClick={saveCcEdit} title={t("Speichern")} style={{ color: "var(--green)" }}><Icon name="check" size={14} /></button>
                      <button type="button" className="icobtn" onClick={() => setCcEdit(null)} title={t("Abbrechen")}><Icon name="x" size={14} /></button>
                    </>
                  ) : (
                    <>
                      <span className="cc-l"><span className="adm-cc-ic"><Icon name="package" size={15} /></span><span className="nm2">{cc.name}</span></span>
                      <button type="button" className="bdg" onClick={() => toggleCc(cc)} title={t("Status umschalten")} style={{ background: "none", border: 0, cursor: "pointer", fontFamily: "inherit" }}>{cc.code}</button>
                      <button type="button" className="icobtn" onClick={() => setCcEdit({ id: cc.id, code: cc.code || "", name: cc.name || "" })} title={t("Bearbeiten")}><Icon name="pencil" size={14} /></button>
                      <button type="button" className="icobtn" onClick={() => delCc(cc)} title={t("Löschen")}><Icon name="trash" size={14} /></button>
                    </>
                  )}
                </div>
              );
